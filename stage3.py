...
# Convert tag word counts to DataFrame
df_tag_words = pd.DataFrame(tag_word_counts).fillna(0).astype(int)

# Calculate TF-IDF scores
tfidf_transformer = TfidfTransformer()
tfidf_matrix = tfidf_transformer.fit_transform(df_tag_words)

# Create DataFrame for TF-IDF scores
df_tfidf = pd.DataFrame(tfidf_matrix.toarray(), index=df_tag_words.index, columns=df_tag_words.columns)
